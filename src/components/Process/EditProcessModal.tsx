...
const EditProcessModal: React.FC<EditProcessModalProps> = ({
  ...
  const [processName, setProcessName] = useState(process.processName);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      const response = await api.post(`/process/edit/${process.id}`, {
        processName,
        processDescription,
      });
        ...
        >
          ...
          <div className="mb-7">
            ...
            {errors.processDescription && (
              <p className="text-red-500 text-xs">{errors.processDescription}</p>
            )}
          </div>
          <div className="flex">
            <button
